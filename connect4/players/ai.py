...
class AIPlayer:
    ...
    def get_expectimax_score(self, state, player_number : int, max_depth : int, step, limit) -> int :
        ...
        for move in l :
            new_state = self.state_update(state, move[0], move[1], player_number)
            move_scores[index] = self.evaluate_expectimax(new_state, step+1)
            sum += max(10, move_scores[index])
            index += 1
            # move_scores[index] = self.get_minimax_score(new_state,3-player_number, max_depth-1, step + 1, limit//number_of_moves)
        # for col in range(columns) :
        #     new_state_push = self.state_update(state, col, 0, player_number)
        ...
        index = 0
        for move in l:
            move_scores[index] = self.get_expectimax_score(new_state,3-player_number, max_depth-1, step + 1, (limit*max(10, move_scores[index]))//sum)
            index += 1
        retval = -1e9
        if player_number == self.player_number:
            for move in move_scores:
    ...
    def get_expectimax_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        for col in range(columns) :
            ...
            new_state_pop = self.state_update(state, col, 1, self.player_number)
            if new_state_push != -1:
                val = self.get_expectimax_score(new_state_push, 3-self.player_number, 10, 0, 1000)
                if val > maax :
                    maax = val
                    move = col
                    isPop = 0 
            if new_state_pop != -1:
                val = self.get_expectimax_score(new_state_pop, 3-self.player_number, 10, 0, 1000)
                if val > maax :
                    maax = val
                    move = col
                    isPop = 1
