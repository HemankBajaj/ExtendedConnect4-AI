...
class Game:
    def __init__(self, player1, player2, time: int, board_init: np.array, m: int, n: int, popout_moves: int):
        ...
        height = m * 200
        width = n * 200
        self.c = tk.Canvas(root, height=height, width=width, xscrollcommand=h.set, yscrollcommand=v.set)
        self.c.pack()
        
        h.config(command=self.c.xview) 
        v.config(command=self.c.yview)
    ...
    def threaded_function(self, arg):
        ...
        for i in range(arg):
            ...
            if self.game_over:
                with open('logs.txt', 'w') as log_file:
                    s = ""
                    s += f'{get_pts(1, self.state[0])}\n'
                    s += f'{get_pts(2, self.state[0])}\n'
                    log_file.write(s)
                    print(s)
                break
